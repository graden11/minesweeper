...
This is the default mode. When a long-running build/download command is run with `run_in_background` and fails, Claude automatically triggers cleanup.
...
```
Claude: Running docker compose up -d --build in background...
[2 min later]
Claude: docker compose build failed (exit code 1, network timeout)
